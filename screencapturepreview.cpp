...
void ScreenCapturePreview::setupUI()
{
...
void ScreenCapturePreview::establishConnections()
{
    ...
    connect(screenCapture, &QScreenCapture::errorOccurred, this, &ScreenCapturePreview::onScreenCaptureErrorOccured, Qt::QueuedConnection);
    connect(windowCapture, &QWindowCapture::errorOccurred, this, &ScreenCapturePreview::onWindowCaptureErrorOccured, Qt::QueuedConnection);
}

void ScreenCapturePreview::onCurrentScreenSelectionChanged(QItemSelection selection)
{
    if (const auto &indexes = selection.indexes(); !indexes.empty())
    {
...
void ScreenCapturePreview::onCurrentWindowSelectionChanged(QItemSelection selection)
{
    if (const auto &indexes = selection.indexes(); !indexes.empty())
    {
        const auto &window = windowListModel->window(indexes.front());

        if (!window.isValid())
        {
            const auto &questionResult = QMessageBox::question(this, tr("Invalid window"), tr("The window is no longer valid. Update the list of windows?"));
...
void ScreenCapturePreview::onWindowCaptureErrorOccured(QWindowCapture::Error, const QString &errorString)
{
    QMessageBox::warning(this, tr("QWindowCapture: Error occurred"), errorString);
}

void ScreenCapturePreview::onScreenCaptureErrorOccured(QScreenCapture::Error, const QString &errorString)
{
...
void ScreenCapturePreview::updateStartStopButtonText()
{
    ...
    case SourceType::Window:
        startStopButton->setText(isActive() ? tr("Stop window capture") : tr("Start window capture"));
        break;
    case SourceType::Screen:
        startStopButton->setText(isActive() ? tr("Stop screen capture")  : tr("Start screen capture"));
        break;
    }
}
